...
class model_template():
    ...
    def __init__(self, Input_prediction_train, Input_path_train, Input_T_train,
                 Output_path_train, Output_T_train, Output_T_pred_train, Output_A_train, Output_T_E_train, 
                 Domain_train, splitter_save_file, num_samples_path_pred): 
        
        if not isinstance(Input_prediction_train, pd.core.frame.DataFrame):
            raise TypeError("Input prediction is expected to be a pandas dataframe.")
        ...
        if not len(Input_prediction_train) == self.num_samples_train:
            raise TypeError("Input prediction is expected to include same number of samples as Input.")
        ...
        for i in range(self.num_samples_train):                
            ...
            for j in range(len(Input_prediction_train.columns)):
                # check if time input consists out of tuples
                if not isinstance(Input_prediction_train.iloc[i,j], np.ndarray):
                    raise TypeError("Input predcition is expected to be consisting of arrays.") 
                # test if input tuples have right length  
                length = len(Input_prediction_train.iloc[i,j])
                if test_length != length:
                    raise TypeError("Input preediction does not have a matching number of timesteps") 
        ...
        # check if Output_T_train is a numpy vector
        if not (isinstance(Output_T_pred_train, np.ndarray) and len(Output_T_pred_train.shape) == 1):
            raise TypeError("Output_T_pred is expected to be a numpy vector.")
        
        # check if the number of samples matches
        if not len(Output_T_pred_train) == self.num_samples_train:
            raise TypeError("Output_T_pred is expected to include same number of samples as Input.")
        ...
        for i in range(self.num_samples_train):                
            ...
            if not isinstance(Output_T_pred_train[i], np.ndarray):
                raise TypeError("Output_T_pred is expected to be consisting of arrays.") 
        ...
        self.Input_prediction_train = Input_prediction_train
        ...
        self.Output_T_pred_train = Output_T_pred_train
    ...
    def predict(self, Input_prediction_test, Input_path_test, Input_T_test, 
                Output_T_pred_test, save_results = True):
        
        # check if input fulfills requirements
        if self.trained == False:
            raise AttributeError('Model has to be trained before prediction.')
        
        # Test if input prediction is a pandas dataframe (needed for input names)
        if not isinstance(Input_prediction_test, pd.core.frame.DataFrame):
            raise TypeError("Input prediction is expected to be a pandas dataframe.")
        ...
        self.num_samples_test = len(Input_path_test)
        ...
        # check if the number of samples matches
        if not len(Input_prediction_test) == self.num_samples_test:
            raise TypeError("Input prediction is expected to include same number of samples as Input.")
        ...
        # check if the number of samples matches
        if not len(Input_T_test) == self.num_samples_test:
            raise TypeError("Input_T_test is expected to include same number of samples as Input.")
            
        # check if Output_T_pred_test is a numpy vector
        if not (isinstance(Output_T_pred_test, np.ndarray) and len(Output_T_pred_test.shape) == 1):
            raise TypeError("Input_T_test is expected to be a numpy vector.")
           
        # check if the Output_T_pred_test of samples matches
        if not len(Output_T_pred_test) == self.num_samples_test:
            raise TypeError("Output_T_pred_test is expected to include same number of samples as Input.")
        ...
        for i in range(self.num_samples_test):            
            ...
            if not isinstance(Output_T_pred_test[i], np.ndarray):
                raise TypeError("Output_T_test is expected to be consisting of arrays.") 
            
            test_length = len(Input_T_test[i])
            
            for j in range(len(Input_prediction_test.columns)):
                # check if time input consists out of tuples
                if not isinstance(Input_prediction_test.iloc[i,j], np.ndarray):
                    raise TypeError("Input predcition is expected to be consisting of arrays.") 
                # test if input tuples have right length  
                length = len(Input_prediction_test.iloc[i,j])
                if test_length != length:
                    raise TypeError("Input preediction does not have a matching number of timesteps") 
        ...
        self.Input_prediction_test = Input_prediction_test
        self.Input_path_test = Input_path_test
        self.Input_T_test = Input_T_test
        
        self.Output_T_pred_test = Output_T_pred_test
