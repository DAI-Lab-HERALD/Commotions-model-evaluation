...
class evaluation_template():
    ...
    def evaluate_prediction(self, Output_pred, model_save_file, create_plot_if_possible = False):
        ...
        if self.get_output_type_class() == 'path':
            ...
            # it might be possible that the Output_path_pred goes above the len of Output_T
            for i in range(self.num_samples):                
                ...
                for j in range(2):
                    ...
                    # Test prediction horizon, cut down if necessary
                    length = Output_path_pred.iloc[i,j].shape[1]
                    if test_length > length:
                        raise TypeError("Output does not have enough timesteps")
